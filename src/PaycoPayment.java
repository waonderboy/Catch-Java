...
public class PaycoPayment implements Payment {
    ...
    private static final String NAME = "PAYCO";
    private static ConcurrentHashMap<String, PayInfo> payLog = new ConcurrentHashMap<>();
    ...
    public static PaycoPayment getInstance() {
        return PaycoPayment.INSTANCE;
    }

    /**
     * 결제가능 여부 체크에 대한 책임은 해당 클래스에 있다! -> 객체지향 5대원칙 SOLID 중 (SRP) Single Responsibility Principal
     */
    @Override
    ...
    public void pay(String requester, double payAmount) {
        System.out.println("페이코페이 ["  + payAmount + "원 ] 결제가 완료되었습니다.");
        payLog.put(getRequesterUniqueKey(requester), PayInfo.of(requester, payAmount));
    }

    @Override
    public void getPayHistory() {
        System.out.println("결제 내역을 보여드리겠습니다.");
